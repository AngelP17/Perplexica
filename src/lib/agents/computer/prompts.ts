...
export const getSwarmPlanningPrompt = (task: string) => {
  return [
    ...
    '- vision: for analyzing screenshots, charts, and other local images with a multimodal model',
    '',
    'Rules:',
    '1. Use the fewest agents possible (1-4 agents maximum)',
    '2. Each agent must have a clear, specific task',
    '3. Only use browser when the task requires actual web interaction',
    '4. Use vision when a screenshot or image needs interpretation, verification, or UI-state inspection',
    '5. Prefer coder for all file and Python work',
    '6. Prefer coder over browser for simple time and timezone questions',
    '',
    'Example 1 - File task:',
    ...
    '',
    'Example 3 - Visual QA task:',
    '{"plan":"Inspect the rendered page visually","agents":[{"role":"browser","task":"Open the target page and capture a screenshot of the relevant state"},{"role":"vision","task":"Analyze the screenshot and report whether the requested UI state is visible"}]}',
    '',
    ...
    '{"plan":"<brief description>","agents":[{"role":"<coder|researcher|browser|vision>","task":"<specific task>"}]}',
  ].join('\n');
};
